...
- Unary `+` converts its operand to a number using `ToNumber` semantics (like `Number(x)`): booleans become `1`/`0`, `null` becomes `0`, a trimmed numeric string parses (non-numeric becomes `NaN`), and non-primitives become `NaN`. Unary `-` and `~` require a number and do not coerce.
...
An assignment target must be a variable or member expression. Compound assignments (`+= -= *= /= %= &= |= ^= <<= >>= >>>=`) follow the rules of their corresponding arithmetic or bitwise operators.
...
- Every supported value provides `toString()`. Objects are encoded as compact, single-line JSON. Array string conversion uses comma-separated element values. Dates convert to their local-time ISO-8601 string with a `±HH:mm` offset (see `Date` below).
...
`Date` represents an instant as milliseconds since the Unix epoch (a number with reference semantics). Calendar access splits into local-time and UTC methods: `getFullYear()`, `getMonth()` (0–11), `getDate()`, `getDay()` (0 = Sunday), `getHours()`, `getMinutes()`, `getSeconds()`, and `getMilliseconds()` return values in the host's local timezone (including daylight-saving adjustments, when applicable), while their `getUTC*` counterparts return UTC. A `Date` also has `getTime()`, `valueOf()`, `getTimezoneOffset()` (the local offset in minutes, with the JavaScript sign — west of UTC positive, so UTC+8 returns `-480`), `toISOString()` (UTC, ending in `Z`), and `toString()` (local wall-clock time rendered as `YYYY-MM-DDTHH:MM:SS.sss±HH:mm`).

`Date.now()` returns the current UTC epoch milliseconds. `Date.UTC(year, month[, ...])` interprets its arguments as UTC and returns epoch milliseconds. The multi-argument `Date(year, month[, day[, ...]])` constructor instead interprets its arguments as **local** wall-clock time (with out-of-range components normalized, e.g. month 12 rolls into January of the next year). `Date.parse(s)` and `new Date(string)` accept ISO-8601 timestamps of the form `YYYY-MM-DD[T| ]HH:MM:SS[.fff]`. A trailing `Z`/`z` denotes UTC; an explicit offset `±HH:mm`, `±HHmm`, or `±HH` shifts the value to UTC; when no offset is present the wall-clock components are interpreted as local time (a bare date `YYYY-MM-DD` denotes local midnight). Unparseable input yields an invalid date. An invalid date stringifies as `"Invalid Date"` and marshals to `null`. `Date` values marshal (including `JSON.stringify` and object/array `.toString()`) as their **local-timezone** ISO string with a `±HH:mm` offset, the same form as `toString()`. (This diverges from JavaScript's `JSON.stringify`, which emits UTC `Z`; use `toISOString()` for a UTC `Z` string.)
